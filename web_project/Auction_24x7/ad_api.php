...
<div class="item">
<div class="">
<img class="img-fluid img-detail" style="max-height: 450px; max-width: 730px;" src="../'.$filename.'" alt="">

</div>
<span class="price"> '.$currency.' '.number_format($price).'</span>
</div>
...
<h2>BIDS</h2>
    ...
    $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  	...
  	$stmt = $conn->prepare("SELECT us.username, b.bid_amt, b.time FROM tbl_bids AS b JOIN tbl_users AS us WHERE b.ad_id = :adid AND b.user_id = us.user_id ORDER BY b.bid_amt DESC LIMIT 5");
...
$cnt=$cnt+1;
	...
	<tr>
  <td colspan="4"><?php  echo "No Bids";?></td>
  </tr>
  <?php
}
...
?>
<div class="widget">
<h4 class="widget-title">Place bid</h4>
<div class="agent-inner">
<form action="../app/send-bid.php" method="POST" autocomplete="OFF">
<input type="hidden" name="userid" value="<?php echo $myid; ?>">
<input type="hidden" name="adid" value="<?php echo $ad_id; ?>">
<input type="hidden" name="highbid" value="<?php echo $highbid; ?>">
<input type="number" name="newbid" required class="form-control" placeholder="Your Bid should Higher than <?php echo $highbid?>">
<button class="btn btn-common fullwidth mt-4">Place Bid</button>
</form>
</div>
</div>
<?php
...
<h4 class="price"> <?php echo $currency; ?> <?php echo number_format($row['price']); ?></h4>
</div>
